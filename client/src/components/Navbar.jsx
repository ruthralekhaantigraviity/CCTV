        ...
        <header className="fixed top-0 left-0 right-0 z-50" style={{ boxShadow: scrolled ? '0 4px 20px rgba(0,0,0,0.1)' : 'none' }}>
            ...
            <div className="hidden md:flex items-center justify-between px-8 py-1.5 text-xs" style={{ background: '#111827' }}>
                ...
                <div className="flex items-center gap-4 text-gray-400">
                    ...
                    <a href="#" aria-label="Facebook" className="hover:text-blue-400 transition-colors"><FaFacebookF size={11} /></a>
                    <a href="#" aria-label="Instagram" className="hover:text-pink-400 transition-colors"><FaInstagram size={11} /></a>
                </div>
            </div>
                ...
                <div className="max-w-[1600px] mx-auto px-6 lg:px-10">
                    <div className="flex items-center h-16 gap-10">
                        ...
                        <Link to="/" className="flex items-center gap-3 group flex-shrink-0">
                            <div className="w-9 h-9 bg-blue-600 rounded-lg flex items-center justify-center group-hover:bg-blue-500 transition-colors shadow-md shadow-blue-500/20">
                        ...
                        <form onSubmit={handleSearch} className="hidden lg:flex items-center border rounded-lg overflow-hidden" style={{ borderColor: isDark ? 'rgba(255,255,255,0.12)' : '#d1d5db', background: isDark ? 'rgba(255,255,255,0.04)' : '#f9fafb' }}>
                            ...
                            />
                            <button type="submit" className="px-3 py-2 text-gray-400 hover:text-blue-500 transition-colors border-l" style={{ borderColor: isDark ? 'rgba(255,255,255,0.1)' : '#e5e7eb' }}>
                                <FiSearch size={16} />
                            </button>
                        </form>
                        ...
                        <div className="hidden lg:flex items-center gap-3">
                            {user ? (
                                <Link to="/dashboard" className="flex items-center gap-1.5 text-sm font-bold text-blue-600 transition-colors hover:text-blue-700">
                                    <FiUser size={14} /> My Account
                                </Link>
                            ) : (
                                <>
                                    <Link to="/login" className="flex items-center gap-1.5 text-sm font-medium transition-colors hover:text-blue-500" style={{ color: 'var(--text-secondary)' }}>
                                        <FiLogIn size={14} /> Login
                                    </Link>
                                    <span style={{ color: 'var(--text-secondary)', opacity: 0.3 }}>|</span>
                                    <Link to="/signup" className="flex items-center gap-1.5 text-sm font-medium transition-colors hover:text-blue-500" style={{ color: 'var(--text-secondary)' }}>
                        ...
                        <Link to="/book-technician" className="hidden lg:inline-flex bg-blue-600 text-white text-xs font-black uppercase tracking-widest px-6 py-2.5 rounded-lg transition-all duration-200 whitespace-nowrap no-hover-bg">
                            Book Now
                        </Link>

                        {/* Mobile buttons */}
                        <div className="lg:hidden flex items-center gap-2 ml-auto">
                            <button className="p-2 rounded-lg hover:bg-gray-500/10 transition-colors" style={{ color: 'var(--text-primary)' }} onClick={() => setMenuOpen(!menuOpen)}>
                    ...
                    >
                        ...
                        <form onSubmit={handleSearch} className="flex items-center border rounded-lg overflow-hidden" style={{ borderColor: isDark ? 'rgba(255,255,255,0.12)' : '#d1d5db' }}>
                            <input
                                ...
                                style={{ color: 'var(--text-primary)' }}
                            />
                            <button type="submit" className="px-3 py-2.5 text-gray-400 hover:text-blue-500 border-l" style={{ borderColor: isDark ? 'rgba(255,255,255,0.1)' : '#e5e7eb' }}>
                                <FiSearch size={16} />
                            </button>
                        </form>
                        ...
                        <div className="space-y-1">
                            {[['/', 'Home'], ['/about', 'About'], ['/products', 'Products'], ['/contact', 'Contact']].map(([path, label]) => (
                                <NavLink key={path} to={path} end={path === '/'} onClick={() => setMenuOpen(false)}
                                    className={({ isActive }) => `block px-3 py-2.5 rounded-lg text-sm font-semibold transition-colors ${isActive ? 'text-blue-600 bg-blue-50 dark:bg-blue-500/10' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5'}`}
                                >
                                    {label}
                                </NavLink>
                            ))}
                        ...
                        <div className="flex flex-col gap-2 pt-2 border-t" style={{ borderColor: isDark ? 'rgba(255,255,255,0.07)' : '#e5e7eb' }}>
                            ...
                            <Link to="/book-technician" onClick={() => setMenuOpen(false)} className="bg-blue-600 text-white text-center py-3 rounded-lg text-sm font-black uppercase tracking-widest">
                                Book a Technician
                            </Link>
                        </div>
                    </motion.div>
