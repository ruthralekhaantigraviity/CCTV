                        ...
                        <div className="bg-card p-8 border border-gray-100 shadow-sm rounded-2xl">
                            <div className="flex flex-col md:flex-row items-center gap-8 mb-10">
                                <div className="relative group">
                                    <div className="absolute inset-0 bg-blue-600/5 rounded-full blur-xl opacity-0 group-hover:opacity-100 transition-opacity" />
                                    ...
                                    <div className="absolute bottom-1 right-1 w-9 h-9 bg-blue-50 dark:bg-blue-500/10 rounded-full flex items-center justify-center text-blue-600 dark:text-blue-400 shadow-sm cursor-pointer hover:bg-white dark:hover:bg-blue-500/20 transition-all z-20 hover:scale-110 border border-blue-100 dark:border-blue-500/20">
                                        <FiEdit3 size={16} />
                                    </div>
                                </div>
                    ...
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-card p-8 border border-gray-100 shadow-sm rounded-2xl">
                        ...
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={order.id} className="group p-5 border border-gray-50 dark:border-gray-800 hover:border-blue-100 dark:hover:border-blue-900 hover:bg-blue-50/10 dark:hover:bg-blue-900/10 rounded-xl transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-6">
                                        ...
                                        <button className="p-2 t-muted hover:text-blue-600 transition-colors"><FiArrowRight /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            {wishlistItems.length > 0 ? (
                                wishlistItems.map(item => (
                                    <div key={item.id} className="flex gap-4 p-4 border border-gray-50 dark:border-gray-800 rounded-2xl hover:shadow-md transition-shadow group">
                                        <div className="relative overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800">
                                            <img src={item.img} alt={item.name} className="w-24 h-24 object-cover group-hover:scale-110 transition-transform duration-500" />
                                        ...
                                        <div className="flex-grow flex flex-col justify-between">
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex gap-3">
                                                    ...
                                                    <button className="text-blue-600 dark:text-blue-400 hover:text-blue-800 transition-colors p-1" title="View Product">
                                ...
                                addresses.map(addr => (
                                    <div key={addr.id} className={`p-6 border ${addr.primary ? 'border-blue-600/30 dark:border-blue-400/30 bg-blue-50/10 dark:bg-blue-900/10' : 'border-gray-50 dark:border-gray-800'} rounded-2xl`}>
                                        ...
                                        <div className="flex gap-4">
                                            <button className="text-xs font-bold t-muted uppercase tracking-widest hover:text-blue-600 transition-colors">Edit</button>
                                            <button
                ...
                return (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card p-8 border border-gray-100 shadow-sm rounded-2xl">
                        ...
                        <div className="mt-10 p-6 bg-red-50 dark:bg-red-950/20 rounded-2xl border border-red-100 dark:border-red-900/40">
                            ...
                            <button className="text-xs font-black uppercase text-red-600 dark:text-red-500 border border-red-200 dark:border-red-900 px-6 py-3 rounded-xl hover:bg-red-600 dark:hover:bg-red-600 hover:text-white transition-all">Deactivate Account</button>
                        </div>
                    </motion.div>
                );
            default:
            ...
            <div className="max-w-[1240px] mx-auto px-4 md:px-8">
                ...
                <div className="flex flex-col lg:flex-row gap-10">
                    ...
                    <aside className="w-full lg:w-64 space-y-1">
                        <div className="pb-6 mb-6 border-b border-gray-200">
                            ...
                            {sidebarItems.map((item) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-4 px-4 py-3 rounded-lg text-sm font-semibold transition-all ${activeTab === item.id ? 'bg-card text-blue-600 shadow-sm border border-gray-100' : 't-primary hover:bg-card hover:shadow-sm'}`}
                                >
                                    <item.icon className={activeTab === item.id ? 'text-blue-600' : 't-muted'} />
                                    {item.label}
                                </button>
                            ))}
                        </div>
                        <button
                            onClick={logout}
                            className="w-full flex items-center gap-4 px-4 py-3 rounded-lg text-sm font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 transition-all"
                        >
                            <FiLogOut /> Logout
                        </button>
                    </aside>
